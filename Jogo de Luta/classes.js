...
class Stage {
    constructor(fighter1, fighter2, fighter1E, fighter2E, logObject) {
    ...
    start() {
        this.update();
        //Todo Evento no botão de Atacar
    ...
    update() {
        //Fighter 1
        this.fighter1E.querySelector('.name').innerHTML = `${this.fighter1.name} - ${this.fighter1.life.toFixed(2)}HP`;
        let f1Pct = (this.fighter1.life / this.fighter1.maxLife) * 100;
        this.fighter1E.querySelector('.bar').style.width = `${f1Pct}%`

        //Fighter 2
        this.fighter2E.querySelector('.name').innerHTML = `${this.fighter2.name} - ${this.fighter2.life.toFixed(2)}HP`;
...
class Log {
    ...
    render() {
        ...
        for(let i in this.list) {
            this.listE.innerHTML += `<li>${this.list[i]}</li>`
        }
    }
}